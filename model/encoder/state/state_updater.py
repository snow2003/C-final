...
class SequenceStateUpdater(StateUpdater):
    ...
    def update_src_dst_user_state(self, user_nodes: Dict[str, List[int]], user_messages: Dict[str, torch.Tensor],
        ...
        for ntype in ['src', 'dst']:
            ...
            updated_state = self.updaters['user'][ntype](user_messages[ntype], state.clone())
    ...
    def update_cas_state(self, unique_node_ids: List[int], unique_messages: torch.Tensor, timestamps: torch.Tensor):
        ...
        updated_state = self.updaters['cas'](unique_messages, state.clone())
    ...
    def check_illegal(self, unique_node_ids: List[int], last_update_time: torch.Tensor, timestamps: torch.Tensor,
        ...
        too_early = last_update_time > timestamps
        if torch.any(too_early):
            bad_ids = torch.tensor(unique_node_ids)[too_early]
            print(f"[Warning] {ntype} time rollback on ids {bad_ids.tolist()}", flush=True)
            # 不再 exit(0); 直接把冲突样本时间强制抬高到 last_update
            timestamps[too_early] = last_update_time[too_early]
