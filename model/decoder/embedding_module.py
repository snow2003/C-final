...
class ConcatEmbedding(EmbeddingModule):
    ...
    def compute_embedding(self, cascades):
        ...
        src_embs, dst_embs = [], []
        for cas in cascades:
            if cas not in cas_interaction or not cas_interaction[cas]:
                # Handle empty interaction case
                emb_dim = self.embedding_dimension
                # Use zeros for source and destination embeddings
                src_emb = torch.zeros(emb_dim, device=self.device)
                dst_emb = torch.zeros(emb_dim, device=self.device)
            else:
...
class AggregateEmbedding(EmbeddingModule):
    ...
    def compute_embedding(self, cascades):
        ...
        if all(l == 0 for l in length):
            # All sequences are empty, return a zero tensor of appropriate shape
            batch_size = len(cascades)
            if self.use_static and self.use_dynamic:
                zero_static = torch.zeros(batch_size, self.embedding_dimension, device=self.device)
                zero_dynamic = torch.zeros(batch_size, self.embedding_dimension, device=self.device)
                return (zero_static, zero_dynamic)
            elif self.use_static:
                return torch.zeros(batch_size, self.embedding_dimension, device=self.device)
        ...
        # Only process valid sequences (length > 0)
        valid_indices = []
        for i, l in enumerate(length):
            if l > 0:
                valid_indices.append(i)
                # Convert history and times to tensors
                cas_history_tensors.append(torch.tensor(cas_history[i], dtype=torch.long, device=self.device))
                cas_times_tensors.append(torch.tensor(cas_times[i], dtype=torch.float, device=self.device))
        ...
        if not valid_indices:
            ...
            if self.use_static and self.use_dynamic:
                return (torch.zeros(batch_size, self.embedding_dimension, device=self.device),
                        torch.zeros(batch_size, self.embedding_dimension, device=self.device))
            elif self.use_static:
                return torch.zeros(batch_size, self.embedding_dimension, device=self.device)
            else:
                return torch.zeros(batch_size, self.embedding_dimension, device=self.device)

        # Use only valid sequences for pad_sequence
        cas_history = pad_sequence(cas_history_tensors, batch_first=True)
        cas_times = pad_sequence(cas_times_tensors, batch_first=True)

        # Adjust length to only include valid sequences
        valid_length = [length[i] for i in valid_indices]
        ...
        if self.use_structural:
            # Only get graphs for valid cascades
            valid_cascades = [cascades[i] for i in valid_indices]
            graph_root, graph_leaf = self.hgraph.get_cas_graph(valid_cascades)
            if graph_root is not None and graph_leaf is not None:
        ...
        if self.use_dynamic:
            valid_cascades = [cascades[i] for i in valid_indices]
            dynamic_emb = self.compute_dynamic_emb(valid_cascades, cas_history, valid_length,
                                                  cas_time_emb, cas_pos_emb, graph_root, graph_leaf)

        if self.use_static:
            valid_cascades = [cascades[i] for i in valid_indices]
            static_emb = self.compute_static_emb(valid_cascades, cas_history, valid_length,
                                               cas_time_emb, cas_pos_emb, graph_root, graph_leaf)

        # Create result tensors of the original batch size
        batch_size = len(cascades)
        if self.use_static and self.use_dynamic:
            full_static_emb = torch.zeros(batch_size, self.embedding_dimension, device=self.device)
            full_dynamic_emb = torch.zeros(batch_size, self.embedding_dimension, device=self.device)

            # Copy computed embeddings to their original positions
            for idx, original_idx in enumerate(valid_indices):
                full_static_emb[original_idx] = static_emb[idx]
                full_dynamic_emb[original_idx] = dynamic_emb[idx]

            return (full_static_emb, full_dynamic_emb)

        elif self.use_static:
            full_static_emb = torch.zeros(batch_size, self.embedding_dimension, device=self.device)
            for idx, original_idx in enumerate(valid_indices):
                full_static_emb[original_idx] = static_emb[idx]
            return full_static_emb

        elif self.use_dynamic:
            full_dynamic_emb = torch.zeros(batch_size, self.embedding_dimension, device=self.device)
            for idx, original_idx in enumerate(valid_indices):
                full_dynamic_emb[original_idx] = dynamic_emb[idx]
            return full_dynamic_emb
