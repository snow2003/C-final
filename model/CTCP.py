...
class CTCP(nn.Module):
    ...
    def forward(self, source_nodes: np.ndarray, destination_nodes: np.ndarray, trans_cascades: np.ndarray,
        ...
        # Convert target_idx to CPU before using for NumPy indexing
        if isinstance(target_idx, torch.Tensor):
            target_idx = target_idx.cpu().numpy()
        target_cascades = trans_cascades[target_idx]

        # ★过滤当前 hgraph 尚未见过的级联
        target_cascades = [c for c in target_cascades if c in self.hgraph.cascades]
        ...
        if len(target_cascades) > 0:
            ...
            # Use original tensor for PyTorch indexing
