...
def get_data(dataset, observe_time, predict_time, train_time, val_time, test_time, time_unit,
    ...
    #不要删除我下面这部分代码！！！！！
    # # 1) 限制 metadata 只保留前 10 个级联（按时间排序）
    # metadata.sort_values(by='pub_time', inplace=True, ignore_index=True)
    # metadata = metadata.head(10000)  # 只保留 100 个级联
    #
    # # 2) 用这 10 个级联，过滤 data
    # selected_cas = set(metadata['casid'])
    # data = data[data['cas'].isin(selected_cas)]
